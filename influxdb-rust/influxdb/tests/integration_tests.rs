...
#[path = "./utilities.rs"]
mod utilities;
use utilities::{assert_result_err, assert_result_ok, create_client, create_db, delete_db, run_test};
...
async fn test_connection_error() {
    ...
    let client = Client::new("http://localhost:10086", test_name).with_auth("nopriv_user", "password");
    let read_query = Query::raw_read_query("SELECT * FROM weather");
    let read_result = client.query(&read_query).await;
    assert_result_err(&read_result);
    match read_result {
        Err(Error::ConnectionError { .. }) => {}
        _ => panic!("Should cause a ConnectionError: {}", read_result.unwrap_err()),
    }
}
...
async fn test_authed_write_and_read() {
    ...
    run_test(
        || async move {
            ...
            client.query(&Query::raw_read_query(query)).await.expect("could not setup db");

            let client = Client::new("http://localhost:9086", TEST_NAME).with_auth("admin", "password");
            let write_query = Timestamp::Hours(11).into_query("weather").add_field("temperature", 82);
            ...
            assert!(!read_result.unwrap().contains("error"), "Data contained a database error");
        },
        || async move {
            let client = Client::new("http://localhost:9086", TEST_NAME).with_auth("admin", "password");
            let query = format!("DROP DATABASE {}", TEST_NAME);

            client.query(&Query::raw_read_query(query)).await.expect("could not clean up db");
        },
    )
    .await;
}
...
async fn test_wrong_authed_write_and_read() {
    ...
    run_test(
        || async move {
            ...
            let client = Client::new("http://localhost:9086", TEST_NAME).with_auth("wrong_user", "password");
            let write_query = Timestamp::Hours(11).into_query("weather").add_field("temperature", 82);
            let write_result = client.query(&write_query).await;
            assert_result_err(&write_result);
            match write_result {
                Err(Error::AuthorizationError) => {}
                _ => panic!("Should be an AuthorizationError: {}", write_result.unwrap_err()),
            }
            ...
            match read_result {
                Err(Error::AuthorizationError) => {}
                _ => panic!("Should be an AuthorizationError: {}", read_result.unwrap_err()),
            }

            let client = Client::new("http://localhost:9086", TEST_NAME).with_auth("nopriv_user", "password");
            let read_query = Query::raw_read_query("SELECT * FROM weather");
            let read_result = client.query(&read_query).await;
            assert_result_err(&read_result);
            match read_result {
                Err(Error::AuthenticationError) => {}
                _ => panic!("Should be an AuthenticationError: {}", read_result.unwrap_err()),
            }
        },
        || async move {
            let client = Client::new("http://localhost:9086", TEST_NAME).with_auth("admin", "password");
...
async fn test_write_and_read_option() {
    ...
    run_test(
        ...
        || async move {
            delete_db("test_write_and_read_option").await.expect("could not clean up db");
        },
    )
    .await;
}
    ...
    run_test(
        || async move {
            ...
            let result = client
                .json_query(query)
                .await
                .and_then(|mut db_result| db_result.deserialize_next_tagged::<WeatherMeta, Weather>());
            assert_result_ok(&result);
...
async fn test_json_query_vec() {
    ...
    run_test(
        || async move {
            ...
            let client = create_client(TEST_NAME);
            let write_query1 = Timestamp::Hours(11).into_query("temperature_vec").add_field("temperature", 16);
            let write_query2 = Timestamp::Hours(12).into_query("temperature_vec").add_field("temperature", 17);
            let write_query3 = Timestamp::Hours(13).into_query("temperature_vec").add_field("temperature", 18);
...
async fn test_serde_multi_query() {
    ...
    run_test(
        || async move {
            ...
            let client = create_client(TEST_NAME);
            let write_query = Timestamp::Hours(11).into_query("temperature").add_field("temperature", 16);
            let write_query2 = Timestamp::Hours(11).into_query("humidity").add_field("humidity", 69);
            ...
            let result = client
                .json_query(Query::raw_read_query("SELECT * FROM temperature").add_query("SELECT * FROM humidity"))
                .await
...
async fn test_wrong_query_errors() {
    let client = create_client("test_name");
    let result = client.json_query(Query::raw_read_query("CREATE DATABASE this_should_fail")).await;
