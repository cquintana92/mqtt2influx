...
fn test_build_query() {
    ...
    assert_eq!(query.get(), "weather_reading,wind_strength=5 pressure=100i 3600000000000");
}
...
async fn test_write_and_read_option() {
    ...
    run_test(
        || async move {
            ...
            let write_result = client.query(&weather_reading.into_query("weather_reading".to_string())).await;
            assert_result_ok(&write_result);
            let query = Query::raw_read_query("SELECT time, pressure, wind_strength FROM weather_reading");
