...
impl DatabaseQueryResult {
    ...
    {
        serde_json::from_value::<Return<T>>(self.results.remove(0)).map_err(|err| Error::DeserializationError {
            error: format!("could not deserialize: {}", err),
        })
    }

    pub fn deserialize_next_tagged<TAG, T: 'static>(&mut self) -> Result<TaggedReturn<TAG, T>, Error>
    where
        TAG: DeserializeOwned + Send,
        T: DeserializeOwned + Send,
    {
        serde_json::from_value::<TaggedReturn<TAG, T>>(self.results.remove(0)).map_err(|err| Error::DeserializationError {
            error: format!("could not deserialize: {}", err),
        })
    }
}
...
impl Client {
    pub async fn json_query(&self, q: ReadQuery) -> Result<DatabaseQueryResult, Error> {
        let query = q.build().map_err(|err| Error::InvalidQueryError { error: format!("{}", err) })?;
        ...
        if !read_query_lower.contains("select") && !read_query_lower.contains("show") {
            let error = Error::InvalidQueryError {
                error: String::from("Only SELECT and SHOW queries supported with JSON deserialization"),
            };
            return Err(error);
        }
        ...
        let body = res.bytes().await.map_err(|err| Error::ProtocolError { error: err.to_string() })?;
        ...
        serde_json::from_slice::<DatabaseQueryResult>(&body).map_err(|err| Error::DeserializationError {
