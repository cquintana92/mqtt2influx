...
impl fmt::Display for Timestamp {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use Timestamp::*;
        match self {
            Nanoseconds(ts) | Microseconds(ts) | Milliseconds(ts) | Seconds(ts) | Minutes(ts) | Hours(ts) => write!(f, "{}", ts),
        }
    }
}
...
impl From<Timestamp> for DateTime<Utc> {
    fn from(ts: Timestamp) -> DateTime<Utc> {
        match ts {
            Timestamp::Hours(h) => {
                let nanos = h * MINUTES_PER_HOUR * SECONDS_PER_MINUTE * MILLIS_PER_SECOND * NANOS_PER_MILLI;
...
#[cfg(test)]
mod tests {
    use super::consts::{MICROS_PER_NANO, MILLIS_PER_SECOND, MINUTES_PER_HOUR, NANOS_PER_MILLI, SECONDS_PER_MINUTE};
    ...
    fn test_equality_string() {
        assert_eq!(ValidQuery::from(String::from("hello")), String::from("hello"));
    }
    #[test]
    ...
    fn test_chrono_datetime_from_timestamp_minutes() {
        ...
        assert_eq!(
            Utc.timestamp_nanos((2 * SECONDS_PER_MINUTE * MILLIS_PER_SECOND * NANOS_PER_MILLI).try_into().unwrap()),
            datetime_from_timestamp
        )
    }
    #[test]
    fn test_chrono_datetime_from_timestamp_seconds() {
        ...
        assert_eq!(
            Utc.timestamp_nanos((2 * MILLIS_PER_SECOND * NANOS_PER_MILLI).try_into().unwrap()),
            datetime_from_timestamp
        )
    }
    #[test]
    ...
    fn test_timestamp_from_chrono_date() {
        ...
        assert_eq!(Timestamp::Nanoseconds(MILLIS_PER_SECOND * NANOS_PER_MILLI), timestamp_from_datetime)
    }
}
